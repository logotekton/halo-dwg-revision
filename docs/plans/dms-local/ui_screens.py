...
def screen_b():
    ...
    b += [T(24, 372, "변경 12 도곽 · 총 변경 23건 · 사소한 변경 41건 접힘", 11, MUTED), BTN(704, 352, 132, 32, "선택 도곽 열기", primary=True, size=12)]
    return SVG(860, 400, "도곽 목록 화면. 전체, 변경, 동일, 신규, 삭제, 짝 없음 필터와 도면번호, 도면명, 전후 리비전, 변경 수 막대, 상태 열이 있는 표. 짝 없음 행에는 수동 짝 맞춤 버튼, 아래에 선택 도곽 열기 버튼", b)
...
def screen_c():
    ...
    # footer
    b += [R(12, 470, 836, 40, fill=CODEBG), BTN(24, 479, 136, 22, "마크업 DWG 만들기", primary=True), BTN(168, 479, 110, 22, "리비전 표 삽입"), BTN(286, 479, 110, 22, "표 텍스트 복사"),
          T(836, 494, "원본은 수정하지 않는다 · 출력은 derivatives 폴더", 10.5, MUTED, "end")]
    return SVG(860, 520, "비교 작업 화면. 왼쪽 캔버스에 변경 전 회색과 변경 후 청록 도면이 겹쳐 있고 다섯 개의 붉은 클라우드 마크에 번호가 붙어 있다. 오른쪽 변경 리스트에는 항목마다 종류, 설명, 승인과 무시 버튼이 있고 아래에 사소한 변경이 접혀 있다. 하단에 마크업 DWG 만들기, 리비전 표 삽입, 표 텍스트 복사 버튼", b)

# ---------- Screen D: 출력 ----------
def screen_d():
    b = [R(150, 20, 560, 290, rx=4, fill="var(--surface)"), T(170, 46, "출력 · A-102 2층 평면도 REV3", 12, INK, weight=600), L(150, 56, 710, 56)]
    b += [T(170, 82, "마크업 DWG", 11, INK, weight=600),
          T(170, 104, "저장 위치", 10.5, MUTED), T(260, 104, r"…\derivatives\A-102_REV3_markup.dwg", 10.5, INK, mono=True),
          T(170, 126, "레이어 · 색", 10.5, MUTED), T(260, 126, "REV-CLOUD · 빨강 · 원 안 번호", 10.5, INK, mono=True)]
    b += [T(170, 158, "리비전 표", 11, INK, weight=600)]
    opts = [("ZWCAD에 바로 삽입", "실행 중인 ZWCAD 감지됨 · COM 연동", True), ("DXF 조각으로 저장", "ZWCAD에서 INSERT로 붙임", False), ("표 텍스트 복사", "엑셀·ZWCAD 표에 붙여넣기", False)]
    for i, (a, c, sel) in enumerate(opts):
        y = 180 + i*22
        b += [f'<circle cx="178" cy="{y-4}" r="5.5" fill="none" stroke-width="1.2" style="stroke:{ACCENT if sel else BEFORE}"/>']
        if sel: b += [f'<circle cx="178" cy="{y-4}" r="3" style="fill:{ACCENT}"/>']
        b += [T(192, y, a, 11, INK, weight=600 if sel else None), T(330, y, c, 10.5, MUTED)]
    b += [T(170, 250, "위치 · 내용", 10.5, MUTED), T(260, 250, "표제란 옆 · 승인 5건 → 리비전 표 5행 (번호·내용·일자·담당)", 10.5, INK)]
    b += [BTN(546, 268, 64, 26, "취소"), BTN(618, 268, 72, 26, "만들기", primary=True)]
    return SVG(860, 330, "출력 대화상자. 마크업 DWG 저장 위치와 레이어, 리비전 표 삽입 방법 세 가지 중 ZWCAD에 바로 삽입이 선택돼 있고, 취소와 만들기 버튼", b)

def fig(svg, cap): return f'  <figure>\n    {svg}\n    <figcaption>{cap}</figcaption>\n  </figure>'

section = "\n".join([
  '  <h2><span class="num">3</span>화면 구상</h2>',
  '  <p>화면은 네 장이다. 세트 지정 → 도곽 목록 → 비교 작업 → 출력 순서로 이어지고, 시간은 대부분 세 번째 화면에서 쓴다. 그림은 배치와 동작을 잡기 위한 초안이라 세부 모양은 개발하면서 바뀐다.</p>',
  '  <h3>화면 A · 세트 지정</h3>',
  fig(screen_a(), "변경 전·후 폴더를 지정하면 XR 폴더와 도곽 수를 바로 읽어 보여준다. 결정 요청 세 가지(사소한 변경 기준, 클라우드 마크 규격, 리비전 표 양식)가 여기 옵션으로 들어간다."),
  '  <h3>화면 B · 도곽 목록</h3>',
  fig(screen_b(), "짝이 맞은 도곽을 변경 많은 순으로 보여준다. 신규·삭제·짝 없음은 따로 표시하고, 짝 없음은 이 화면에서 수동으로 맞춘다."),
  '  <h3>화면 C · 비교 작업 화면</h3>',
  fig(screen_c(), "왼쪽은 전(회색)·후(청록) 겹쳐 보기와 클라우드 마크, 오른쪽은 변경 리스트다. 번호를 누르면 그 자리로 확대되고 항목마다 승인·무시를 정한다. 사소한 변경은 접혀 있고, 출력 버튼은 승인 항목만 내보낸다."),
  '  <h3>화면 D · 출력</h3>',
  fig(screen_d(), "승인한 항목만 마크업 DWG와 리비전 표로 나간다. ZWCAD가 실행 중이면 COM 연동으로 바로 삽입하고, 아니면 DXF 조각이나 표 텍스트로 넘긴다."),
  '  <p class="note">부가 화면으로 도곽 일괄 편집(날짜·리비전 기호)이 있다. 도곽 목록에서 여러 도곽을 골라 표제란 값을 한 번에 바꾸는 단순한 표 화면이라 그림은 생략한다.</p>',
  ''])
