...
- **재레이어링 복제 블록(R1-06b).** 뷰어는 가시성과 색을 그려지는 엔티티 단위로 판단하는데 INSERT·DIMENSION이 그리는 것은 블록 정의 안의 도형이라, 참조만 `__CMP_*`로 옮기면 §2의 두 약속(보기 모드 숨김, 빨강·시안)이 문·창·치수에 대해 성립하지 않는다. 그래서 `__CMP_ADDED`·`__CMP_REMOVED`에 놓인 INSERT와 DIMENSION은 원래 블록 대신 **복제 블록** `__CMPA_<이름>`·`__CMPR_<이름>`을 가리킨다. `<이름>`은 비교 DXF 안의 이름이므로 전 도면 블록은 `__B_`를 유지한다(`__CMPR___B_DOOR_900`). 복제 블록 안의 모든 엔티티는 그 비교 레이어에 `color = 256(BYLAYER)`·선종류·선굵기 BYLAYER이고 `true_color`는 지운다. 중첩 INSERT는 재귀적으로 같은 쪽 복제 블록을 가리키며(자기 참조는 순환 가드로 끊는다), DIMENSION의 익명 기하 블록(`*D…`)도 같은 방식으로 복제해 `geometry`가 복제본을 가리킨다. 이름 중간의 `*`는 `_`로 접고(`*D5` → `__CMPA__D5`), 255자를 넘으면 `__CMPA_<앞 200자>_<sha1 앞 8자>`로 접는다. 복제는 **(블록, 쪽)당 하나**이고 그 쪽의 모든 참조가 공유한다. 원래 레이어에 남는(변경 없는) INSERT·DIMENSION과 마크업 DWG는 대상이 아니다. XDATA(§4)는 참조에만 달고 복제 블록 안에는 달지 않으므로 `handle_to_cluster`(§7)는 이 규칙과 무관하게 그대로다.
