...
function mleaderDetail(entity: AcDbMLeader): CadEntityDetail {
  ...
  for (const leader of entity.leaders) {
    for (const line of leader.leaderLines) {
      ...
      if (last) {
        ...
        if (!previous || previous.x !== tail.x || previous.y !== tail.y || previous.z !== tail.z) {
          points.push(tail);
        }
      }
  ...
  const contents: unknown = textGroup?.properties
    ...
    ?.accessor.get();
  const contentKind =
    entity.contentType === 1 ? 'BLOCK' : entity.contentType === 2 ? 'MTEXT' : 'NONE';
  return {
    kind: 'MLEADER',
    leaderLines:
...
export async function openDxfDatabase(
  ...
  const dwgVersion =
    typeof version === 'object' && version !== null && 'name' in version
      ? String((version as { name: unknown }).name)
      : (acadver ?? 'AC1032');
