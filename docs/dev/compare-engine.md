...
6. `__CMP_*`에 놓인 INSERT·DIMENSION을 **재레이어링 복제 블록**으로 돌린다(아래 절).
7. 헤더·CLASSES 고정 → `audit()` → 직렬화 → 텍스트 정규화 → 바이트로 저장.

`handle_to_cluster`는 이 과정에서 **새 문서의 핸들**로 채운다(클라우드·배지 도형·배지 숫자·
히트 사각형 + 그 클러스터의 변경이 `__CMP_*`에 쓴 엔티티). 뷰어는 히트 테스트로 얻은 핸들을
이 표에 넣어 클러스터를 찾는다.

## 블록 가시성 복제 (R1-06b, `_RelayeredBlocks`)

뷰어는 가시성과 색을 **그려지는 엔티티 단위**로 판단한다. 그런데 INSERT가 그리는 것은 블록
정의 안의 도형이고, 그 도형들은 참조를 `__CMP_ADDED`로 옮겨도 여전히 `A-DOOR` 레이어에
자기 색으로 남아 있다. 그래서 R1-08이 화면 C에서 실측한 결과는 계약 §2의 두 약속이 문·창에
대해서만 통째로 깨진 상태였다 — "전" 보기에서 신설된 문이 안 숨고, 빨강·시안으로도 안 보인다.
DIMENSION도 같다(그리는 것은 익명 `*D` 기하 블록이다). 문·창 이동은 실무에서 가장 흔한
변경이라 뷰어가 아니라 **엔진이** 고친다(CLAUDE.md 4).

`__CMP_ADDED`·`__CMP_REMOVED`에 놓인 INSERT·DIMENSION은 자기 블록의 **복제본**을 가리키게
바꾼다. 복제본 안의 엔티티는 전부 그 비교 레이어에 `color=256`·`linetype=BYLAYER`·
`lineweight=-1`이고 `true_color`는 지운다(24비트 색은 `color`보다 우선해서, 남겨 두면 BYLAYER가
안 먹는다).

| 항목 | 규칙 |
|---|---|
| 이름 | `__CMPA_<이름>`(ADDED) / `__CMPR_<이름>`(REMOVED). `<이름>`은 **비교 DXF 안의 이름**이라 전 도면 블록은 `__B_`를 달고 있다 → `__CMPR___B_DOOR_900` |
| `*` | 이름 중간의 `*`는 DXF가 금지한다. `prefix_before_blocks`와 같이 `_`로 접는다 → `*D5` → `__CMPA__D5` |
| 길이 | 255자를 넘으면 `__CMPA_<앞 200자>_<sha1 앞 8자>` |
| 중첩 | 복제본 안의 INSERT·DIMENSION은 자기 블록의 같은 쪽 복제본을 가리킨다(재귀) |
| 순환 | 이름을 **먼저** 기록하고 내용을 훑는다. 자기 자신에 닿는 블록은 무한 재귀 대신 이미 기록된 이름을 만난다 |
| 개수 | (블록, 쪽)당 하나. 참조 6개가 한 복제본을 공유한다 — 파일 크기는 바뀐 블록 **종류** 수에 비례한다 |
| XDATA | 복제본 안에는 안 단다. 히트 테스트가 잡는 것은 참조 자신이고, 클러스터는 그 핸들로 찾는다(계약 §7) |
| 기준점·플래그·단위 | 원본 그대로 |

바뀌지 않은 INSERT·DIMENSION은 원래 블록 그대로다(안 움직인 문 5개는 여전히 `DOOR_900`을
그린다). 마크업 DWG(R1-09)는 대상이 아니다 — 레이어를 옮기지 않으니 복제할 것도 없다.

**조립 마지막 단계인 이유.** 복제는 클라우드·배지를 다 그린 뒤에 돈다. 그래야 모델공간 핸들
발급 순서가 R1-06 그대로여서 `handle_to_cluster`가 한 글자도 안 바뀐다. 복제 요청은 만들기
전에 `(레이어, 블록 이름)`으로 정렬하므로 블록 테이블 순서도 참조가 놓인 순서가 아니라 비교
자체의 성질이다(계약 §8).

크기 대가는 S02(문 1개 이동)에서 66,227 → 67,785바이트(+2.4%), S03(치수 1개)에서
76,313 → 80,327바이트다.
...
| 전/후 보기에서 문·창·치수가 안 숨거나 색이 원래 색 | 블록 가시성 복제(`relayer_block_references`). 참조가 `__CMPA_`·`__CMPR_` 블록을 가리키는지 본다 |
