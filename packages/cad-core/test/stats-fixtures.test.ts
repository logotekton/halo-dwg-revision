    ...
    try {
      ...
      if (testCase.lengthGap === 'ellipse-and-spline') {
        ...
        for (const entity of model?.entities() ?? []) {
          if (entity.type === 'ELLIPSE') ellipse += entity.curveLength() ?? 0;
          if (entity.type === 'SPLINE') spline += entity.curveLength() ?? 0;
        }
        // GEOM-PHTM holds the two SPLINEs and six POINTs, so its truth
        // `length_sum` is exactly ezdxf's flattened spline length.
        ...
        expectedLength = truthTotals.length_sum + ellipse + (spline - truthSpline);
      }
      ...
      for (const bucket of modelBuckets) {
        ...
        if (isExactBucket(bucket.aggregate.count_by_type)) {
          expect(
