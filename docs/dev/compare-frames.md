...
mac에서는 acad-ts가 ATTRIB을 잃어서(스파이크 §3.2, 68장 중 45장) 기본 경로가 대부분 0을 내지만,
블록명 경로는 **읽힌 파일에 대해 장부 수와 정확히 맞는다**. **합격 판정은 Windows 설치본(ZWCAD
변환)에서 한다** — `docs/gates/R1.md`.

2026-09-04 측정(mac, acad-ts, 18분 1초):

| 공종 | 읽힌 파일 | 기본 | 블록명 | 미인식 | 장부 | 변환 s |
|---|---:|---:|---:|---:|---:|---:|
| 03_전기 | 0 / 1 | 0 | 0 | 0 | 104 | — |
| 02_기계 | 1 / 1 | 36 | **51** | 0 | 51 | 268.3 |
| 04_통신 | 1 / 1 | 0 | **28** | 1 | 28 | 70.4 |
| 05_소방_기계 | 1 / 2 | 0 | 16 | 1 | 45 | 81.8 |
| 06_소방_전기 | 1 / 1 | 0 | **28** | 1 | 28 | 81.5 |
| 01_건축 | 36 / 37 | 0 | 94 | 36 | 99 | 186.3 |

건너뛴 파일 3개는 전부 변환 단계 문제이고 도곽 추출과는 무관하다.

- `03_전기/…전기도서 (1).dwg` — acad-ts `dwg2dxf` 실패(20.8 MB). 따로 돌리면 343초에 변환은
  되지만 만들어진 작업용 DXF를 ezdxf가 못 읽는다(아래와 같은 결함).
- `05_소방_기계/02_기계소방내진도면….dwg`, `01_건축/A-060 실내외재료마감표, 상세도.dwg` —
  작업용 DXF에 `90` / `None`이 적혀 있어 `DXFStructureError: Invalid tag (code=90, value="None")`.
  MULTILEADER의 `LEADER_LINE` 안이다.

`assign_entities` 실측(30초 예산):

| 파일 | 최상위 엔티티 | 시간 |
|---|---:|---:|
| 02_기계 …_수정.DWG | 127,676 | 3.86 s |
| 06_소방_전기 …전기소방도서 (3).dwg | 124,487 | 2.64 s |
| 04_통신 …통신도서.dwg | 70,550 | 2.55 s |
| 05_소방_기계 01_기계소방도면….dwg | 48,291 | 1.17 s |

엔티티당 약 30 µs이므로 35만 엔티티는 11초 안팎이다. 다만 비용은 최상위 엔티티 수보다
**블록 정의 안의 내용**에 더 좌우된다(그래서 블록 메모가 필요했다).
