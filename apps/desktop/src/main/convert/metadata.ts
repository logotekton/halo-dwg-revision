...
export async function readDwgMetadata(
  ...
  try {
    // `--xrefs` is what adds the two arrays (W3-06's `info` keeps them opt-in
    // so callers that only want version/entity_count pay nothing).
    const args = [acadBridgeEntry, 'info', dwgPath, '--xrefs']
