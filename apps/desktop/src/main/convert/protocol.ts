 ...
 *
 * The shape is exactly what `acad-bridge info --xrefs` emits
 * (`packages/acad-bridge/src/acad/xref-style-scan.ts`), which is the contract's
 * `{block_name, path}` (`docs/contracts/wave-3.md` "계약 갱신").
 */
 ...
 * (W3-05) needs it to map Korean TTF names. `null` when the style has none.
 */
