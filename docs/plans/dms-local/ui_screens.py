...
def screen_b():
    ...
    b += [T(24, 372, "변경 12 도곽 · 승인 23건 · 미결 2건 · 사소한 변경 41건 접힘", 11, MUTED), BTN(556, 352, 132, 32, "선택 도곽 열기", size=12), BTN(704, 352, 132, 32, "세트 전체 출력…", primary=True, size=12)]
    return SVG(860, 400, "도곽 목록 화면. 전체, 변경, 동일, 신규, 삭제, 짝 없음 필터와 도면번호, 도면명, 전후 리비전, 변경 수 막대, 상태 열이 있는 표. 짝 없음 행에는 수동 짝 맞춤 버튼, 아래에 선택 도곽 열기 버튼과 세트 전체 출력 버튼", b)
...
def screen_c():
    ...
    # footer
    b += [R(12, 470, 836, 40, fill=CODEBG), BTN(24, 479, 124, 22, "세트 전체 출력…", primary=True), BTN(156, 479, 116, 22, "이 도곽만 출력…"), BTN(280, 479, 110, 22, "표 텍스트 복사"),
          T(836, 494, "출력은 승인 항목만 · 원본은 수정하지 않는다", 10.5, MUTED, "end")]
    return SVG(860, 520, "비교 작업 화면. 왼쪽 캔버스에 변경 전 회색과 변경 후 청록 도면이 겹쳐 있고 다섯 개의 붉은 클라우드 마크에 번호가 붙어 있다. 오른쪽 변경 리스트에는 항목마다 종류, 설명, 승인과 무시 버튼이 있고 아래에 사소한 변경이 접혀 있다. 하단에 세트 전체 출력, 이 도곽만 출력, 표 텍스트 복사 버튼", b)
...
def screen_d():
    ...
    b += radio(X+90, 84, "세트 전체", "변경 12 도곽 · 승인 23건 (미결 2건 제외)", True)
    b += radio(X+400, 84, "이 도곽만", "A-102 · 승인 5건", False)
    ...
    return SVG(860, 380, "출력 대화상자. 범위는 세트 전체가 기본이고 이 도곽만도 고를 수 있다. 마크업 DWG는 변경된 도곽마다 한 파일, 리비전 표는 도곽별 표와 전체 변경 리스트 두 가지, 넣는 방법은 ZWCAD에 바로 삽입, DXF 조각, 표 텍스트 복사 중 선택. 취소와 만들기 버튼", b)

def fig(svg, cap): return f'  <figure>\n    {svg}\n    <figcaption>{cap}</figcaption>\n  </figure>'

section = "\n".join([
  '  <h2><span class="num">3</span>화면 구상</h2>',
  '  <p>화면은 네 장이다. 세트 지정 → 도곽 목록 → 비교 작업 → 출력 순서로 이어지고, 시간은 대부분 세 번째 화면에서 쓴다. 출력은 도곽 하나가 아니라 세트 전체가 기본이다. 그림은 배치와 동작을 잡기 위한 초안이라 세부 모양은 개발하면서 바뀐다.</p>',
  '  <h3>화면 A · 세트 지정</h3>',
  fig(screen_a(), "변경 전·후 폴더를 지정하면 XR 폴더와 도곽 수를 바로 읽어 보여준다. 결정 요청 세 가지(사소한 변경 기준, 클라우드 마크 규격, 리비전 표 양식)가 여기 옵션으로 들어간다."),
  '  <h3>화면 B · 도곽 목록</h3>',
  fig(screen_b(), "짝이 맞은 도곽을 변경 많은 순으로 보여준다. 신규·삭제·짝 없음은 따로 표시하고, 짝 없음은 이 화면에서 수동으로 맞춘다. 검토가 끝나면 여기서 세트 전체를 한 번에 출력한다."),
  '  <h3>화면 C · 비교 작업 화면</h3>',
  fig(screen_c(), "왼쪽은 전(회색)·후(청록) 겹쳐 보기와 클라우드 마크, 오른쪽은 변경 리스트다. 번호를 누르면 그 자리로 확대되고 항목마다 승인·무시를 정한다. 사소한 변경은 접혀 있다. 도곽을 하나씩 검토한 뒤 세트 전체 출력으로 넘어간다."),
  '  <h3>화면 D · 출력</h3>',
  fig(screen_d(), "출력은 세트 전체가 기본이다. 변경된 도곽 전부의 마크업 DWG와 전체 변경 리스트(도곽별 묶음)를 한 번에 만들고, 필요할 때만 한 도곽만 출력한다. ZWCAD가 실행 중이면 COM 연동으로 바로 삽입하고, 아니면 DXF 조각이나 표 텍스트로 넘긴다."),
  ''])
