...
1. **기본 변환기는 mlightcad `AcDbDatabase.dxfOut()`으로 정한다**(ADR-0002 §3의 1·2순위를 맞바꾼다).
   결정타는 속도도 메모리도 아니고 **충실도**다: acad-ts가 쓴 DXF는 **엔진(ezdxf)이 아예 읽지 못하고**
   (`AttributeError` / `ZeroDivisionError`로 죽는다), ATTRIB을 전부 잃는다. `dxfOut()` 산출물은 엔진이 읽고,
   차이는 **그룹코드 두 개 누락**(INSERT의 `66`, HATCH 경계의 External 비트)뿐이며 데이터는 온전하다.
2. **acad-ts는 100만 엔티티에서 힙과 무관하게 죽는다**(`RangeError: Maximum call stack size exceeded`,
   16 GB 힙에서도 동일). `dxfOut()`은 같은 파일을 8.3 s / RSS 4.6 GB로 통과한다.
3. **두 변환기 모두 "성공"을 반환하면서 데이터를 잃는다.** libredwg-web은 20만 엔티티 DWG에서
   **85개만 돌려주면서 `lastOpenError`가 `null`**이었다. → 임포트는 stats 교차검증을 **게이트**로 걸어야 한다.
4. **임계값은 MB가 아니라 엔티티 수로 건다.** 같은 도면이 DXF 41 MB / DWG 5.3 MB로 8배 차이 난다.
   A(≤25만, 전체 편집) / B(25만~80만, 라이트 DXF) / C(>80만, 엔진 전용). 크기는 1차 추정에만 쓴다.
5. **네이티브 LibreDWG `dwg2dxf`는 미측정**(Homebrew 설치 필요, 이 태스크의 정책상 설치하지 않음).
6. **실제 도면 샘플이 없다.** `samples/`는 비어 있고 DWG 픽스처는 전부 acad-ts가 쓴 것이라
   "DWG 읽기 품질" 비교의 기준으로 쓸 수 없다(§4.3). **G0에서 실제 DWG 확보를 요청한다.**
...
- **시간·피크 RSS**: Node/Python 프로세스는 `/usr/bin/time -l`의 `real` / `maximum resident set size`.
  Chromium은 드라이버 프로세스의 자손 중 **Chromium 실행 파일인 것만**(브라우저+렌더러+GPU+워커 유틸리티) 골라 그 RSS 합을 150 ms 간격으로 `ps -axo pid=,ppid=,rss=,comm=`으로 샘플링해 최댓값을 쓴다(Node·Vite는 제외된다). 렌더러 단일 프로세스 최댓값도 따로 기록한다.
...
| F12.dxf | browser | Chromium 기본 | 8.7 s (파싱 3 635 ms + `dxfOut` 2 352 ms) | **4 456 MB**(트리) / **4 248 MB**(렌더러 단일) | ok · 1 000 026 엔티티 전부, 244.6 MB 출력 |
...
- `dxfOut()`은 100만 엔티티를 기본 힙 안에서 통과하지만 RSS 4.6 GB다 —
  **16 GB 기기에서 이 크기는 위험 구간**이고, 브라우저에서는 **렌더러 단일 프로세스가 4.25 GB**로
  Chromium의 렌더러 V8 힙 상한(약 4 GB) 바로 앞까지 간다. 여기서 파일이 조금만 더 커지면 렌더러가 죽는다.

### 3.4 Chromium 메모리 스케일링 (브리프의 "16 GB 시뮬레이션 대신 파일 크기 축소")

Chromium에는 밖에서 힙 상한을 줄 수단이 없으므로 파일 크기를 키워가며 한계를 봤다.

| DXF 입력 | 엔티티 | Chromium 트리 피크 RSS | 렌더러 단일 프로세스 | 도면 순비용(트리 − 기준선 348 MB) |
|---|---:|---:|---:|---:|
| F06.dxf 0.07 MB | 86 | 354 MB | — | 약 6 MB |
| F11.dxf 41.4 MB | 200 006 | 1 433 MB | — | 약 1 085 MB |
| F12.dxf 209.3 MB | 1 000 026 | 4 383–4 456 MB | **4 248 MB** | 약 4 060 MB |

엔티티당 약 **4.1 KB**로 거의 선형이다. 렌더러 단일 프로세스가 100만 엔티티에서 **4.25 GB**로
Chromium의 렌더러 V8 힙 상한(약 4 GB) 경계에 닿는다 → **브라우저 상한은 대략 100만 엔티티**이고,
안전 마진을 두면 §5.3의 B/C 경계 80만이 나온다. 이 값은 기기 RAM이 24 GB든 16 GB든 같다(렌더러 상한은 고정).

그 과정에서 **하네스 쪽 한계**를 하나 만났고, 수치가 아니라 방법에 영향을 주므로 기록한다:
페이지가 만든 DXF를 `POST /__sink/`로 되돌려줄 때 Chromium이 그 본문을 CDP
`Network.requestWillBeSent`에 그대로 실어 보내는데, 244 MB 본문에서 Playwright 전송 계층이
Node의 512 MB 문자열 상한을 넘어 `ERR_STRING_TOO_LONG`으로 드라이버째 죽었다.
그래서 이제 산출물이 64 MB를 넘으면 크기·시간만 재고 바이트는 되돌리지 않는다(`SINK_LIMIT_BYTES`).
F12의 충실도 수치는 그 가드를 넣기 전 실행이 **POST를 끝낸 뒤** 드라이버가 죽으면서 디스크에 남긴
브라우저 산출 DXF(244 582 254 B)로 검사한 값이고, Node 경로 산출물과 바이트 수·통계가 전부 같다.
가드가 들어간 지금 재실행하면 그 셀은 시간·크기만 재고 충실도는 Node 경로로 확인해야 한다.
...
### 5.1 **결정 1 — 기본 변환기는 mlightcad `AcDbDatabase.dxfOut()`이다.** acad-ts는 보조로 내린다.

| 기준 | acad-ts (`packages/acad-bridge`) | **mlightcad `dxfOut()`** | 네이티브 LibreDWG |
|---|---|---|---|
| 라이선스 | MIT | data-model **MIT** (+ DWG 읽기는 libredwg GPL, `packages/dwg-io-gpl` 격리) | GPL |
| **엔진(ezdxf)이 산출 DXF를 읽는가** | **아니오** — F03/F06/F10_grid/F11에서 예외로 죽는다 | **예** — 모든 픽스처 |
| 데이터 손실 | ATTRIB·ATTDEF 전량 삭제(태그 없음), 블록명=레이어명 INSERT 드롭, 중복 핸들 | **없음.** 그룹코드 2개 누락(`66`, HATCH `92`의 External 비트) |
| 20만 엔티티 | 3.3 s / 1.2 GB (DXF 읽기), DWG→DXF 15.7 s / 2.3 GB | 1.5 s / 1.4 GB |
| **100만 엔티티** | **실패 — `RangeError: Maximum call stack size exceeded`, 16 GB 힙으로도 동일** | 8.3 s / 4.6 GB, 전량 보존 |
| DXF→DWG 쓰기 | 20만에서 358 s | (없음 — DWG 쓰기 미지원) |
| Node(utilityProcess)에서 DWG 읽기 | **가능** (유일) | **불가** — `Worker` 전역 필요 |
| 한글 텍스트 | 정확(W2-05 디코드 수정 후) | 정확 |

근거의 우선순위는 **충실도 > 안정성 > 속도 > 메모리**다. acad-ts는 첫 두 항목에서 탈락한다:
산출물을 엔진이 아예 못 읽고(ADR-0002 §2의 "뷰어와 엔진이 같은 바이트를 읽는다"가 성립하지 않는다),
100만 엔티티에서 사용자가 손쓸 수 없는 방식으로 죽는다.

`dxfOut()`의 결함 두 개는 **누락된 플래그 두 개**이고 데이터는 파일 안에 온전하다.
`packages/dwg-io-gpl`(또는 변환 래퍼)에서 산출 DXF를 후처리해 (a) ATTRIB이 뒤따르는 INSERT에 `66 1`을 넣고
(b) HATCH 경계 경로의 첫 루프에 External 비트를 세우면 F06·F11·F12 모두 truth와 일치한다.
상류(mlightcad)에도 보고할 것.

**acad-ts는 유지하되 역할을 바꾼다:**
- **DWG 읽기 폴백** — 브라우저/워커를 못 쓰는 환경(순수 Node utilityProcess, CI)에서 DWG를 여는 유일한 MIT 경로.
  단, **그 산출 DXF를 엔진에 바로 먹이지 않는다.** ATTRIB/MTEXT 수복 후처리를 통과해야 한다.
- **제3의 파서 교차검증**(ADR-0002 §6) — 이 태스크에서 실제로 가장 값어치가 있었던 용도다.
- **DXF→DWG 내보내기** — 유일한 DWG 라이터. 20만 엔티티에 358 s이므로 백그라운드 작업으로만.

### 5.2 **결정 2 — "변환 성공"은 stats 교차검증을 통과해야 성공이다**

libredwg-web이 F11.dwg에서 200 006개 중 85개만 돌려주면서 `lastOpenError`는 `null`이었다(§4.4).
acad-ts는 ATTRIB을 조용히 망가뜨렸다(§4.1). **두 변환기 모두 "성공"을 반환하면서 데이터를 잃는다.**
따라서 임포트 파이프라인은 다음을 **게이트**로 둔다(경고가 아니라 차단):

1. 변환기 산출 DXF를 엔진이 `ezdxf.readfile`로 열지 못하면 → 그 변환기 실패로 처리하고 다음 후보로 폴백.
2. 감사기(`ezdxf.recover`)의 **삭제(fix) 건수가 0이 아니면** 실패로 처리(acad-ts의 ATTRIB 삭제가 여기 걸린다).
3. 변환기 자신이 보고한 엔티티 수와 엔진 `stats.totals.entity_count`가 **±0.5 % 밖이면** 실패로 처리.
   (`dxfOut()`의 고아 SEQEND는 후처리로 없어지므로 이 문턱을 넘지 않는다.)

### 5.3 **결정 3 — ADR-0002 §4의 60 / 200 / 600 MB를 아래 실측값으로 대체한다**

크기 임계값은 형식마다 다르고, 실제 게이트 변수는 엔티티 수다(§5.4).

| 티어 | 게이트(엔티티 수, 1차) | 원본 크기 환산(2차, 참고) | 동작 | 근거 |
|---|---|---|---|---|
| **A. 브라우저 미리보기 + 전체 편집** | ≤ **250 000** | DXF ≤ 55 MB / DWG ≤ 8 MB | 지금 방식 그대로 | F11(20만)이 Chromium 1.43 GB·1.2 s, Node 1.4 GB·1.5 s. 16 GB 기기의 1 GB 힙 모사에서도 통과 |
| **B. 라이트 DXF(뷰어는 축약본, 엔진은 정본)** | 250 000 ~ **800 000** | DXF 55–170 MB / DWG 8–25 MB | 뷰어에는 도곽·레이어 서브셋만, 엔진은 전체 | 100만에서 `dxfOut()` RSS 4.6 GB — 24 GB 기기는 되지만 16 GB 기기·브라우저 렌더러 힙(≈4 GB)에는 위험 |
| **C. 엔진 전용(썸네일 + 시트 추출)** | > 800 000 | DXF > 170 MB / DWG > 25 MB | 브라우저에 통째로 올리지 않는다. ezdxf가 74 s·2.4 GB로 처리 | ezdxf는 100만에서도 2.4 GB로 가장 가볍다. 브라우저 경로는 이 크기에서 측정 자체가 불안정하다(§3.4) |

- ADR-0002의 60 MB 미리보기 상한은 **DXF 기준으로는 과하게 보수적**이다(55 MB/20만 엔티티가 1.2초에 열린다).
  반대로 **DWG 기준으로는 지나치게 관대하다**: 60 MB DWG는 엔티티 200만 개 규모이고 그 구간은 위 C 티어다.
  → 단일 MB 임계값을 폐기하고 형식별 + 엔티티 수 2단 게이트로 바꾼다.
- 200 MB / 600 MB 경계도 같은 이유로 엔티티 수 기준으로 옮긴다.

### 5.4 게이트를 무엇으로 걸 것인가 (크기 vs 엔티티 수)
...
- 문구 초안(ko.json 키는 W3-02가 정한다):
  - 1단 B티어: "이 도면은 큽니다(약 N만 개 요소). 화면에는 축약본을 열고 물량은 원본으로 계산합니다."
  - 1단 C티어: "이 도면은 화면에 통째로 열기에 너무 큽니다. 도곽 단위로 나눠 열거나, 물량 계산만 진행할 수 있습니다."
  - 2단(예상보다 큰 경우): "실제 요소 수가 예상보다 많아(N만 개) 축약본으로 전환했습니다."
- **RAM으로 해결되지 않는다는 것도 문구에 반영한다.** acad-ts의 100만 실패는 스택 한계라 메모리를 늘려도
  똑같이 실패한다. "메모리가 부족합니다"류 문구를 쓰면 사용자가 잘못된 대응(RAM 증설)을 하게 된다.

### 5.5 임계값을 크기로 추정할 때 쓰는 실측 밀도

| 형식 | B/엔티티 (F11) | B/엔티티 (F12) |
|---|---:|---:|
| DXF (ASCII, R2018) | 207 | 209 |
| DWG (acad-ts 쓰기) | 26 | — |
| `dxfOut()` 산출 DXF | 242 | 245 |

같은 도면의 DXF:DWG 비가 약 **8:1**이다. ADR-0002의 단일 MB 임계값이 성립할 수 없는 이유가 이 숫자다.

## 6. 남은 것

1. **실제 도면으로 재보정.** 이 문서의 수치는 전부 합성 픽스처다. 실무 DWG(외주 XREF, 프록시 엔티티,
   OLE, 이미지, 커스텀 객체)에서는 파서별 결함 분포가 달라질 수 있다. G0 질문 참조.
2. **네이티브 LibreDWG `dwg2dxf` 미측정.** `brew install libredwg` 후 `tools/bench-open.mjs`에
   `native` 경로(`runTimed('dwg2dxf', [in, out])`)를 한 함수 추가하면 같은 표에 들어온다.
   설치는 `sudo`/시스템 설치 금지 규칙 때문에 이 태스크에서 하지 않았다.
...
5. **뷰어(WebGL) 렌더 경로는 측정하지 않았다.** 위 브라우저 수치는 전부 파싱 + `dxfOut()`이고,
   three.js 지오메트리·머티리얼 비용은 빠져 있다. 하네스에 `--browser-render`(→ `AcApDocManager.openDocument`)
   가 준비돼 있으니 W3-02가 프레임 예산까지 포함해 A/B 경계를 재확인해야 한다. 렌더 비용이 붙으면
   §5.3의 A 경계(25만)는 내려갈 수 있다.
